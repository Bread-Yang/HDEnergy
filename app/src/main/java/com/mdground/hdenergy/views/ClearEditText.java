...
/**
 * 这个类是可清除EditText内容，提供接口ClearEditListener，开发者只要实现这个接口
 *
...
public class ClearEditText extends EditText implements
    ...
    // 能输入的最大金额
    private float mMaxInputFee;

    public interface ClearEditListener {
        public void clearEdit();
    }

    private ClearEditListener clearEditListener;
    ...
    public void setClearPasswordListener(ClearEditListener clearEditListener) {
        this.clearEditListener = clearEditListener;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            if (getCompoundDrawables()[2] != null) {
                ...
                if (touchable) {
                    this.setText("");
                    if (null != clearEditListener) {
                        clearEditListener.clearEdit();
                    }
                    return false;
                }
            }
    ...
    public float getmMaxInputFee() {
        return mMaxInputFee;
    }

    public void setmMaxInputFee(float mMaxInputFee) {
